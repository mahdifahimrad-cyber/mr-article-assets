...
/* v8 — on a fresh article entry, position the viewport on the article title once. */
(function () {
    if (window.__mehdiradArticleTitleEntryScrollReady) return;
    window.__mehdiradArticleTitleEntryScrollReady = true;
    ...
    function getTopOverlayOffset() {
        ...
        Array.prototype.forEach.call(document.body.children, function (element) {
            if (!element || element.classList.contains('article-container')) return;
    ...
    function positionArticleTitle() {
        if (completed || cancelledByUser || !isFreshPageEntry()) return;

        var title = document.querySelector('.article-container .article-title');
        if (!title) return;
        ...
        var topOffset = getTopOverlayOffset() + 24;
        var targetTop = Math.max(0, currentScroll + titleRect.top - topOffset);
    ...
    function scheduleTitlePosition() {
        ...
        window.requestAnimationFrame(function () {
            window.requestAnimationFrame(positionArticleTitle);
        });
    }

    function beginTitlePositioning() {
    ...
    if (document.readyState === 'complete') {
        beginTitlePositioning();
    } else {
        window.addEventListener('load', beginTitlePositioning, {
            once: true,
            passive: true
        });
    }
