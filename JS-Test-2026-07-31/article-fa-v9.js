...
(function () {
    ...
    function hideUniversalCta(container) {
        ...
        nextPaint(function () {
            ...
            const geometry = getMorphGeometryFromRects(visualRect, edgeRect);

            if (closeButton) {
                const closeRect = closeButton.getBoundingClientRect();
                closeButton.style.transformOrigin =
                    ((visualRect.left + visualRect.width / 2) - closeRect.left) + 'px ' +
...
/* v9 — on a fresh article entry, reveal the full hero below the site header once. */
(function () {
    if (window.__mehdiradArticleHeroEntryScrollReady) return;
    window.__mehdiradArticleHeroEntryScrollReady = true;

    var cancelledByUser = false;
    var completed = false;
    var settlingTimer = 0;

    function cancelEntryScroll() {
        cancelledByUser = true;
        window.clearTimeout(settlingTimer);
    }

    ['wheel', 'touchstart', 'pointerdown'].forEach(function (eventName) {
        window.addEventListener(eventName, cancelEntryScroll, {
            once: true,
            passive: true,
            capture: true
        });
    ...
    window.addEventListener('keydown', function (event) {
        ...
        if (keys.indexOf(event.key) !== -1) cancelEntryScroll();
    }, { once: true, capture: true });

    function isFreshPageEntry() {
        ...
        try {
            ...
            if (navigation && navigation.type === 'back_forward') return false;
        } catch (error) {}
    ...
    function getTopOverlayOffset() {
        ...
        Array.prototype.forEach.call(document.querySelectorAll('body *'), function (element) {
            if (!element || element.closest('.article-container')) return;
            ...
            var isTopOverlay = rect.top <= 3
                && rect.bottom > 0
                && rect.height > 0
                && rect.height <= 240
                && rect.width >= viewportWidth * 0.45;

            if (isTopOverlay) offset = Math.max(offset, rect.bottom);
        });

        return offset;
    }

    function positionArticleHero() {
        if (completed || cancelledByUser || !isFreshPageEntry()) return;

        var hero = document.querySelector('.article-container .article-hero');
        if (!hero) return;
        ...
        /* Keep the blue hero visibly below the site header. */
        var topOffset = getTopOverlayOffset() + 18;
        var targetTop = Math.max(0, currentScroll + heroRect.top - topOffset);
    ...
    function scheduleAfterLayoutSettles() {
        if (completed || cancelledByUser || !isFreshPageEntry()) return;

        window.requestAnimationFrame(function () {
            window.requestAnimationFrame(function () {
                settlingTimer = window.setTimeout(positionArticleHero, 180);
            });
        });
    }

    function beginHeroPositioning() {
        ...
        ]).then(scheduleAfterLayoutSettles);
    }

    if (document.readyState === 'complete') {
        beginHeroPositioning();
    } else {
        window.addEventListener('load', beginHeroPositioning, {
            once: true,
            passive: true
        });
    }
